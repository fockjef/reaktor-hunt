...
const maze = fs.readFileSync("xennial-3-maze.txt").toString().split(/\n/).map((row) => row.split(""));
// find start, goal, key and treasure nodes in maze
const nodes = [];
maze.forEach((row, r) => row.forEach((type, c) => /[<>a-z$]/.test(type) && nodes.push({ id: nodes.length, type, r, c })));
// calculate distances between nodes for every possible key combination
const allKeys = nodes.filter(n => /[a-z]/.test(n.type)).map(n => n.type).sort();
const distance = {};
